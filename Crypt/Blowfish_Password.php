...
class Crypt_Blowfish_Password
{
    ...
    {
		////////////////////////////////////////////////
		/* 		Disable First Encrypting 			  */
        /* return $this->_crypt->encrypt($plainText); */
		////////////////////////////////////////////////
		
			srand((double) microtime() * 1000000); //for sake of MCRYPT_RAND
			$KeyPass = md5($KeyPass); //to improve variance
			////////////////////////////////////////////////////////////////
			/* Open module, and create IV */
			////////////////////////////////////////////////////////////////
			$td = mcrypt_module_open('des', '','cfb', '');
			$KeyPass = substr($KeyPass, 0, mcrypt_enc_get_key_size($td));
			...
			////////////////////////////////////////////////////////////////
			if (mcrypt_generic_init($td, $KeyPass, $iv) != -1) {
				///////////////////
				/* Encrypt data */
				///////////////////
					$c_t = mcrypt_generic($td, $this->_crypt->encrypt($plainText));
    ...
    {
		/////////////////////////////////////////////////
		/* 		Disable First Decryption 			   */
        /* return $this->_crypt->decrypt($cipherText); */
		/////////////////////////////////////////////////
		
		/** Micro -> Base64 */
		$Base64Dec = base64_decode($cipherText);
			...
			$KeyPass = substr($KeyPass, 0, mcrypt_enc_get_key_size($td));
			$iv_size = mcrypt_enc_get_iv_size($td);
			$iv = substr($Base64Dec,0,$iv_size);
			$string = substr($Base64Dec,$iv_size);
			////////////////////////////////////////////////////////////////
			/* Initialize encryption handle */
			////////////////////////////////////////////////////////////////
		...
		if (mcrypt_generic_init($td, $KeyPass, $iv) != -1) {
			...
			$c_t = mdecrypt_generic($td, $string);
					...
					mcrypt_module_close($td);
			//return $c_t;
			return $this->_crypt->decrypt($c_t);
		}
